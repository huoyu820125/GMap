...
static unsigned int DefaultHash(unsigned char* key, unsigned int keySize)
{
	int i = 0;
...
static unsigned int DJBHash(const unsigned char *buf, int len) 
{
...
bool RHTable::Expand(unsigned long size)
{
	...
	{
		if ( m_pHashTable->size * 4 > m_maxHash ) size = m_maxHash;
		else size = m_pHashTable->size * 4;
	}
...
{
	return HashFunction( (unsigned char*)&key, sizeof(mdk::uint8) );
}

unsigned int RHTable::RemoteHash( mdk::uint16 key )
{
	return HashFunction( (unsigned char*)&key, sizeof(mdk::uint16) );
}

unsigned int RHTable::RemoteHash( mdk::uint32 key )
{
	return HashFunction( (unsigned char*)&key, sizeof(mdk::uint32) );
}

unsigned int RHTable::RemoteHash( mdk::uint64 key )
{
	return HashFunction( (unsigned char*)&key, sizeof(mdk::uint64) );
}

unsigned int RHTable::RemoteHash( unsigned char *key, unsigned int keySize )
{
	return HashFunction(key, keySize);
}
...
RHTable::ELEMENT* RHTable::Find(unsigned char* key, unsigned int size, unsigned int hashValue,  bool bInsert )
{
	...
	if ( !m_bRemote ) hashValue = HashFunction(key, size);
